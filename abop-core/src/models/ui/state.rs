...
impl AppState {
    ...
    fn initialize_components(&mut self) {
        ...
        self.persistence = Some(
            StatePersistence::new()
                .unwrap_or_else(|_| StatePersistence::with_path(PathBuf::from("state.toml"))),
        );
    ...
    pub fn save_with_options(&self, options: SaveOptions) -> Result<()> {
    ...
    pub fn recover(&mut self) -> Result<()> {
        if let Some(ref validator) = self.validator {
            ...
            if validation_result.has_critical_issues() {
                ...
                warn!(
                    "State has critical issues that need recovery: {}",
                    validation_result
                );
    ...
    #[must_use] pub fn get_summary(&self) -> String {
        format!(
...
impl AppState {
    /// Get current view
    #[must_use] pub const fn current_view(&self) -> &ViewType {
        &self.current_view
    }

    /// Get selected audiobook ID
    #[must_use] pub const fn selected_audiobook_id(&self) -> Option<&String> {
        self.selected_audiobook_id.as_ref()
    }

    /// Get user preferences
    #[must_use] pub const fn user_preferences(&self) -> &UserPreferences {
        &self.user_preferences
    }

    /// Get window configuration
    #[must_use] pub const fn window_config(&self) -> &WindowConfig {
        &self.window_config
    }

    /// Get theme configuration
    #[must_use] pub const fn theme_config(&self) -> &ThemeConfig {
        &self.theme_config
    }

    /// Get playback configuration
    #[must_use] pub const fn playback_config(&self) -> &PlaybackConfig {
        &self.playback_config
    }

    /// Get application data
    #[must_use] pub const fn app_data(&self) -> &AppData {
        &self.app_data
    }
}
