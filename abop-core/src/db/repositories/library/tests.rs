...
mod library_tests {
    ...
    use crate::test_utils::{TestDatabase, TestAssertions};
    use std::path::Path;
    ...
    fn test_create_library_duplicate_name() {
        ...
        repo.create(name, path1).expect("First library creation should succeed");

        // Try to create second library with same name
        let result = repo.create(name, path2);
        assert!(result.is_err(), "Creating library with duplicate name should fail");
    ...
    fn test_create_library_duplicate_path() {
        ...
        // Create first library
        repo.create(name1, path).expect("First library creation should succeed");

        // Try to create second library with same path
        let result = repo.create(name2, path);
        assert!(result.is_err(), "Creating library with duplicate path should fail");

        // Verify it's a constraint error
        let error_msg = result.unwrap_err().to_string().to_lowercase();
        assert!(
            error_msg.contains("unique") || error_msg.contains("duplicate") || 
            error_msg.contains("constraint") || error_msg.contains("path"),
    ...
    fn test_find_all_empty_database() {
        ...
        assert!(libraries.is_empty(), "Empty database should return no libraries");
    }
    ...
    fn test_find_all_with_libraries() {
        ...
        let lib1 = libraries.iter().find(|lib| lib.name == "Library 1").unwrap();
    ...
    fn test_update_library_success() {
        ...
        // Update the library
        let success = repo.update(&library_id, "Updated Name", Path::new("/updated/path")).unwrap();
    ...
    fn test_update_library_not_found() {
        ...
        let result = repo.update("nonexistent-id", "New Name", Path::new("/new/path")).unwrap();
    ...
    fn test_find_by_name_case_sensitivity() {
        ...
        assert!(found.is_some(), "Should find existing library by exact name");
        
        // Different case should not find it
        let found = repo.find_by_name("existing library").unwrap();
        assert!(found.is_none(), "Should not find library with different case");
    }
    ...
    fn test_find_by_path_exact_match() {
        ...
        assert!(found.is_some(), "Should find existing library by exact path");
        
        let found = repo.find_by_path("/nonexistent/path").unwrap();
        assert!(found.is_none(), "Should not find nonexistent library by path");
    }
    ...
    fn test_library_count_via_find_all() {
        ...
        let libraries = repo.find_all().unwrap();
        assert_eq!(libraries.len(), 2, "Should count all libraries via find_all");
    }
    ...
    fn test_very_long_name() {
        ...
        match result {
            ...
            Err(error) => {
                let error_msg = error.to_string().to_lowercase();
                assert!(
                    error_msg.contains("length") || error_msg.contains("size") || error_msg.contains("long"),
