...
fn test_database_path_consistency() -> Result<()> {
    ...
    let db_path = Database::get_app_database_path()
        .context("Failed to get app database path")?;
...
fn test_database_operations() -> Result<()> {
    ...
    if table_exists == 0 {
        ...
        let migrations_exist: i64 = conn.query_row(
    ...
    for (i, library) in libraries.iter().enumerate() {
        println!("  {}. {} (ID: {}) - Path: {}", 
                 i + 1, library.name, library.id, library.path.display());
    }
...
fn test_library_operations() -> Result<()> {
    ...
    println!("Testing library creation with path: {}", test_path.display());
    
    // Check if this library already exists
    match db.find_library_by_path(&test_path)? {
        Some(existing) => {
            println!("📖 Found existing library: {} (ID: {})", 
        ...
        None => {
            ...
            let library_id = db.add_library_with_path("Validation Test Library", test_path.clone())
            ...
            println!("    Test path as string_lossy: '{}'", test_path.to_string_lossy());
            ...
            let found_via_sql = stmt.query_row([path_str.as_ref()], |row| {
                Ok((
                    row.get::<_, String>(0)?,
                    row.get::<_, String>(1)?,
                    row.get::<_, String>(2)?
            ...
            let created_library = db.find_library_by_path(&test_path)?
                .context("Library not found after creation")?;
            
            println!("✅ Successfully retrieved created library: {}", created_library.name);
            
            // Test library repository operations
            let repo = db.libraries();
            
            // Find by ID
            let found_by_id = repo.find_by_id(&library_id)
            ...
            // Find by name
            let found_by_name = repo.find_by_name("Validation Test Library")
    ...
    let all_libraries = db.get_libraries()
        .context("Failed to get all libraries")?;
...
mod tests {
    ...
    fn test_database_path_consistency() {
        ...
        assert!(result.is_ok(), "Database path consistency test failed: {:?}", result);
    }
    
    #[test]
    fn test_database_operations() {
        let result = super::test_database_operations();
        assert!(result.is_ok(), "Database operations test failed: {:?}", result);
    }
}
