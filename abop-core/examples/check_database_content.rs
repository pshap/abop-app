...
fn main() -> Result<()> {
      ...
      for (i, library) in libraries.iter().enumerate() {
        ...
        println!("     Audiobooks: {}", audiobooks.len());
          for (j, audiobook) in audiobooks.iter().enumerate() {
            println!("       {}. {}", j + 1, audiobook.title.as_deref().unwrap_or("Unknown Title"));
            println!("          Author: {}", audiobook.author.as_deref().unwrap_or("Unknown"));
    ...
    let total_audiobooks = repo.find_all()
