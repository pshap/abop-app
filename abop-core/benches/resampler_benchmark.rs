...
fn generate_test_audio_with_signal(
    ...
    let data: Vec<f32> = match signal_type {
        AudioSignalType::WhiteNoise => (0..total_samples)
        ...
        AudioSignalType::MultiTone => {
            ...
            (0..total_samples)
                .map(|i| {
                    ...
                    let harmonic2 =
                        (2.0 * std::f32::consts::PI * base_freq * 2.0 * time).sin() * 0.2;
                    let harmonic3 =
                        (2.0 * std::f32::consts::PI * base_freq * 3.0 * time).sin() * 0.1;
        ...
        AudioSignalType::SpeechLike => {
            ...
            (0..total_samples)
                .map(|i| {
                    ...
                    let formant2 =
                        (2.0 * std::f32::consts::PI * (base_freq * 2.5) * time).sin() * 0.2;
...
fn bench_resampler(c: &mut Criterion) {
    ...
    let signal_types = [
        ...
        AudioSignalType::SineWave,   // Pure tone for testing filter accuracy
    ];
    ...
    for (src_rate, dst_rate, channels, duration) in test_cases.iter() {
        for signal_type in signal_types.iter() {
            let buffer =
                generate_test_audio_with_signal(*src_rate, *channels, *duration, *signal_type);
