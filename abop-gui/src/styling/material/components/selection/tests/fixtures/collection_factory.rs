...
pub fn multiple_select_collection(labels: &[&str]) -> ChipCollection {
    ...
    builder
        .build()
        .expect("Failed to build multiple-select collection")
}
...
pub fn display_only_collection(labels: &[&str]) -> ChipCollection {
    ...
    builder
        .build()
        .expect("Failed to build display-only collection")
}
...
pub fn uniform_size_collection(
    mode: ChipSelectionMode,
    size: crate::styling::material::components::selection::ComponentSize,
) -> ChipCollection {
    ChipCollectionBuilder::new(mode)
        .size(size)
...
pub fn pre_selected_collection(
    ...
    let collection = filter_collection(labels);
...
pub fn max_selected_collection(mode: ChipSelectionMode) -> ChipCollection {
    ...
    match mode {
        ...
        ChipSelectionMode::Single => pre_selected_collection(mode, labels, &[0]),
...
pub fn single_item_collections() -> Vec<ChipCollection> {
    ALL_SELECTION_MODES
        ...
        .map(|&mode| {
            ...
            builder
                .build()
                .expect("Failed to build single-item collection")
        })
        .collect()
}
