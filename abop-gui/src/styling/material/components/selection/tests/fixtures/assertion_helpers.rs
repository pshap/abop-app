...
use crate::styling::material::components::selection::{
    Chip, ChipCollection, ChipVariant, ChipState, ComponentSize, 
    ChipSelectionMode, SelectionError,
};
use super::test_data::*;
...
pub fn assert_collection_selection(collection: &ChipCollection, expected_count: usize) {
    ...
    assert_eq!(
        ...
        expected_count, 
        actual_count,
        collection.selected_chips().iter().map(|c| c.label()).collect::<Vec<_>>()
    );
}
...
pub fn assert_collection_selection_mode(collection: &ChipCollection, expected_mode: ChipSelectionMode) {
    assert_eq!(
...
pub fn assert_empty_label_error(result: Result<Chip, SelectionError>) {
    match result {
        ...
        Err(SelectionError::InvalidLabel { reason }) => {
            assert!(
                reason.contains("empty") || reason.contains("must have") || reason.contains("required"),
        ...
        Err(other_error) => {
            panic!(
                "Expected empty label error, but got: {:?}", 
                other_error
            );
        }
        Ok(chip) => {
...
pub fn assert_invalid_state_error(result: Result<(), SelectionError>, expected_details_pattern: &str) {
    match result {
...
/// Assert multiple chips have the same property
...
pub fn assert_collections_equivalent(collection1: &ChipCollection, collection2: &ChipCollection) {
    ...
    for (i, (chip1, chip2)) in collection1.chips().iter().zip(collection2.chips().iter()).enumerate() {
        assert_eq!(
            chip1.label(), 
            chip2.label(),
