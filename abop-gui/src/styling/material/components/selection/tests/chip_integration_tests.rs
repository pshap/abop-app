...
use crate::styling::material::components::selection::{
    ChipBuilder, ChipCollection, ChipCollectionBuilder, ChipSelectionMode, ChipVariant,
};
...
mod integration_tests {
    ...
    fn test_large_collection_performance() {
        ...
        assert!(creation_time.as_millis() < 100, "Collection creation should be fast");
        ...
        assert!(selection_time.as_millis() < 50, "Selection operations should be fast");
    ...
    fn test_none_selection_mode_behavior() {
        ...
        assert!(result.is_err(), "None selection mode should reject selection attempts");
    ...
    fn test_state_persistence_across_operations() {
        ...
        let initial_state: Vec<bool> = collection.chips()
            .iter()
            .map(|c| c.is_selected())
            .collect();
        ...
        let restored_state: Vec<bool> = collection.chips()
            .iter()
            .map(|c| c.is_selected())
            .collect();
