...
impl fmt::Display for ButtonError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            ButtonError::UnsupportedVariant(variant) => write!(f, "Unsupported variant: {}", variant),
            ButtonError::MissingLabelAndIcon => write!(f, "Button must have either a label or an icon"),
