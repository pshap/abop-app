...
fn test_status_display_invalid_processing_progress() {
    ...
    // Test with negative processing progress (should clamp to 0.0)
    let mut params = create_base_params();
    params.processing_audio = true;
    ...
    // Test with processing progress > 1.0 (should clamp to 1.0)
    let mut params = create_base_params();
    params.processing_audio = true;
...
fn test_status_display_edge_cases() {
    ...
    // Minimal state with just processing complete
    ...
    max_values.scan_progress = Some(ScanProgress::FileProcessed {
        current: usize::MAX,
        total: usize::MAX,
        file_name: "a".repeat(1000),
        progress_percentage: 1.0,
    });
    max_values.cached_scan_progress_text =
        Some("Scanning with very long text that should be properly truncated or wrapped in the UI");
    ...
    max_values.processing_status =
        Some("Status with very long text that should be properly truncated or wrapped in the UI");
    ...
    // Paused again with current file
    let mut paused_state = create_base_params();
    paused_state.player_state = PlayerState::Paused;
    paused_state.current_playing_file = Some(&test_audio_path);
    let _element = StatusDisplay::enhanced_view(paused_state, &tokens);
    ...
    // Out-of-bounds progress handling
...
/// Helper to create baseline params
