...
fn audiobook_table_handles_selections() {
    ...
    let selection_scenarios = [
        ...
        (HashSet::from(["book1".to_string(), "book3".to_string()]), "multiple selection"),
        (HashSet::from(["book1".to_string(), "book2".to_string(), "book3".to_string()]), "all selected"),
        (HashSet::from(["nonexistent".to_string()]), "invalid selection"),
    ];
...
fn audiobook_table_performance_large_dataset() {
    ...
    let large_selection: HashSet<String> = (0..1000)
        .step_by(100)
        .map(|i| format!("id_{i}"))
        .collect();
    
    let table_state = TableState::default();
    let element = AudiobookTable::view(&large_audiobook_set, &large_selection, &table_state, &tokens);
    let _ = element;
}
