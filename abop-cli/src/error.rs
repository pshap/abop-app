...
pub fn validate_library_path(path: &Path) -> CliResult<()> {
    ...
    let metadata = std::fs::metadata(path).map_err(|e| match e.kind() {
        std::io::ErrorKind::NotFound => {
            anyhow::anyhow!("Library path does not exist: {}", path.display())
        }
        _ => anyhow::anyhow!("Cannot access library path {}: {}", path.display(), e),
    })?;
...
pub fn validate_existing_database_path(path: &Path) -> CliResult<()> {
    ...
    let metadata = std::fs::metadata(path).map_err(|e| match e.kind() {
        std::io::ErrorKind::NotFound => {
            anyhow::anyhow!("Database file does not exist: {}", path.display())
        }
        _ => anyhow::anyhow!("Cannot access database file {}: {}", path.display(), e),
    })?;
    ...
    fn test_cli_result_extensions() {
        ...
        assert!(
            result
                .unwrap_err()
                .to_string()
                .contains("Database operation 'test operation' failed")
        );
        ...
        assert!(
            result
                .unwrap_err()
                .to_string()
                .contains("Library scan operation failed")
        );
    }
}
