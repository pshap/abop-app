...
fn output_json_error(error: &anyhow::Error) {
    ...
    let output = crate::output::CliOutput::error(error_chain[0].clone(), error_code, context);

    match output.to_json() {
        Ok(json) => {
            log::debug!(
                "Serialized error output to JSON, size: {} bytes",
                json.len()
            );
            println!("{json}");
        }
        Err(serialization_error) => {
            log::error!(
                "Failed to serialize error output to JSON: {}",
                serialization_error
            );
...
fn categorize_error(error_message: &str) -> String {
    ...
    match error_lower {
        msg if msg.contains("does not exist") || msg.contains("not found") => {
            "PATH_NOT_FOUND".to_string()
        }
        msg if msg.contains("is a directory") || msg.contains("invalid path") => {
            "INVALID_PATH".to_string()
        }
        msg if msg.contains("database") || msg.contains("sqlite") => "DATABASE_ERROR".to_string(),
        msg if msg.contains("permission") || msg.contains("access denied") => {
        ...
        msg if msg.contains("serializ") || msg.contains("json") => {
